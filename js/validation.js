...
formSignUp.addEventListener("submit",function(e){
    ...
    function validName(Name){
        ...
        if(userNameReg.test(Name)){
            ...
            error[0].innerHTML = "username tidak valid";
            e.preventDefault();
        }
    }
    function validEmail(Email){
        ...
        if(emailReg.test(Email)){
            ...
            error[1].innerHTML = "email tidak valid";
            e.preventDefault();
        }
    }
    function validPass(Pass){
        ...
        if(passReg.test(Pass)){
            ...
            error[2].innerHTML = "password tidak valid";
            e.preventDefault();
        }
    }
    validName(userName);
    ...
    validPass(pass);
    if(!(pass == conPass)){
        error[3].innerHTML = "konfirmasi password tidak valid";
        e.preventDefault();
    }
    else{
        error[3].innerHTML = "";
